function [state,t,X] =  FeatureExtraction_plus(edfaddress,hypaddress)
Data = AnnotExtract(hypaddress);
[hdr, record] = edfread(edfaddress);
...
[b2,a2]=butter(6,0.96);
for kk=1:5
record(kk,:) = filter(b2,a2,record(kk,:));
for jj=2800000:length(record(kk,:))
    record(kk,jj)=0;
end
end
figure(1)
